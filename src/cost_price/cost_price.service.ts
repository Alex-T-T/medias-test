...
export const calculateCostPrices = async (
    invoice: incomingInvoice | outgoingInvoice
) => {
    ...
    if (monthsDifference > 0) {
        ...
        for (let i = 0; i <= monthsDifference; i++) {
            ...
            products.forEach(async (product) => {
                await calculateMonthlyCostPrice(
                    product.product_id,
                    targetMonthStart
                );
            });
...
export const calculateMonthlyCostPrice = async (
    id: string,
    invoiceMonthStart: Date
) => {
    ...
    invoicePreviousMonthStart.setMonth(
        invoicePreviousMonthStart.getMonth() - 1
    );

    // 1. Загальна вартість залишку товару
    const lastMonthCostPrice = (await getLastMonthCostPrice(
        id,
        previousMonthEnd
    )) as unknown as cost_price;

    const remainingIncomeQuantity =
        (await incomingInvoiceProductsService.getRemainingIncomeQuantity(
            id,
            invoicePreviousMonthStart,
            previousMonthEnd
        )) || 0;

    const remainingOutQuantity =
        (await outgoingInvoiceProductsService.getRemainingOutQuantity(
            id,
            invoicePreviousMonthStart,
            previousMonthEnd
        )) || 0;

    const totalQuantityOnPreviousMonth =
        remainingIncomeQuantity - remainingOutQuantity;

    const totalCostForPreviousMonth =
        (await incomingInvoiceProductsService.getCurrentMonthIncomingCost(
            id,
            invoicePreviousMonthStart,
            previousMonthEnd
        )) as number;
    ...
    const totalProductCost =
        lastMonthCostPrice && lastMonthCostPrice.value
            ? lastMonthCostPrice.value * totalQuantityOnPreviousMonth
            : totalQuantityOnPreviousMonth * totalCostForPreviousMonth;

    // 2. Весь прихід товару за поточний місяць
    const currentMonthIncomingQuantity =
        (await incomingInvoiceProductsService.getCurrentMonthIncomingQuantity(
            id,
            invoiceMonthStart,
            invoiceMonthEnd
        )) || 0;
    ...
    const currentStock =
        totalQuantityOnPreviousMonth + currentMonthIncomingQuantity;

    // 4. Загальна вартість товару, який поступив за цей місяць
    const currentMonthIncomingCost =
        (await incomingInvoiceProductsService.getCurrentMonthIncomingCost(
            id,
            invoiceMonthStart,
            invoiceMonthEnd
    ...
    const currentCostPrice =
        (totalProductCost + currentMonthIncomingCost) / currentStock;
...
export const getCostPriceByDate = async (id: string, date: Date) => {
    ...
    const costPrice = await CostPrice.findOne({
        where: {
            id,
