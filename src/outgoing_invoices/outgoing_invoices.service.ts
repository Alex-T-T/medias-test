...
import { outgoingInvoice, outgoingInvoiceCreate } from "./outgoing_invoices.schemas"
...
export const getAllInvoices = async () => {
    ...
    const invoices = await OutgoingInvoice.findAll({include: [{
...
export const getInvoiceById = async (id: string) => {
    ...
    const invoice = await OutgoingInvoice.findOne({where:{ id}, include: [{
        model: OutgoingInvoiceProduct,
        as: 'products'
    }]})
...
export const createNewInvoice = async (createDTO: outgoingInvoiceCreate) => {
    ...
    try {
        ...
        const newInvoice = await OutgoingInvoice.create({
            date: createDTO.date,
        }, { transaction }) as unknown as typeof OutgoingInvoice & { id: string };
        ...
        const invoiceWithRelations = await getInvoiceById(newInvoice.id) as unknown as outgoingInvoice
